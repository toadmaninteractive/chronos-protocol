    ...
    public void Generate(ErlModel model, Module module)
    {
        foreach (var form in module.Structs)
        {
            ...
            if (enabled)
            {
                ...
                erl.Export(form.erlName, 1);
                erl.Function(string.Format(func, form.erlName, form.erlRemoteType, Igor.Erlang.Json.JsonSerialization.ParseJsonFun(form.erlJsonTag(form), erl.Name)));
